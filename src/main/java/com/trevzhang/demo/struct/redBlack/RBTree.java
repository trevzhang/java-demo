...
public class RBTree<K extends Comparable<K>, V> {
    ...
    /**
     * 插入后修复红黑树平衡的方法 |---情景1：红黑树为空树 |---情景2：插入节点的key已经存在 |---情景3：插入节点的父节点为黑色
     * <p>
     * 情景4 需要咱们去处理 |---情景4：插入节点的父节点为红色 |---情景4.1：叔叔节点存在，并且为红色（父-叔 双红） |---情景4.2：叔叔节点不存在，或者为黑色，父节点为爷爷节点的左子树
     * |---情景4.2.1：插入节点为其父节点的左子节点（LL情况） |---情景4.2.2：插入节点为其父节点的右子节点（LR情况） |---情景4.3：叔叔节点不存在，或者为黑色，父节点为爷爷节点的右子树
     * |---情景4.3.1：插入节点为其父节点的右子节点（RR情况） |---情景4.3.2：插入节点为其父节点的左子节点（RL情况）
     */
    ...
    /**
     * 左旋方法 左旋做了几件事？ 1.将y的左子节点赋值给x的右边，并且把x设置为y的左子节点的父节点 2.将x的父节点（非空时）指向y，更新y的父节点为x的父节点 3.将y的左子节点指向x，更新x的父节点为y
     */
    ...
    /**
     * 右旋方法 右旋都做了几件事？ 1.将x的右子节点 赋值 给了 y 的左子节点，并且更新x的右子节点的父节点为 y 2.将y的父节点（不为空时）指向x，更新x的父节点为y的父节点 3.将x的右子节点指向y，更新y的父节点为x
     */
